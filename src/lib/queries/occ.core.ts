...
async function buildLocationRollups(context: AuthContext): Promise<LocationRollup[]> {
...
export async function fetchEstateRollup(context: AuthContext) {
...
export async function fetchBranchPack(context: AuthContext, locationId: string): Promise<BranchPack> {
  ...
  const rollups = await buildLocationRollups(context);
  const rollup = rollups.find((r) => r.location_id === locationId);
...
export async function fetchHandoverDigest(context: AuthContext, locationId: string): Promise<HandoverDigest> {
...
export async function fetchHandovers(context: AuthContext, locationId: string) {
...
export async function fetchExceptionsFeed(context: AuthContext): Promise<ExceptionItem[]> {
  const now = new Date().toISOString();
