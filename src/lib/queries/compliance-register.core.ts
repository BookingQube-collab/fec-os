import { daysRemaining, venueMatchesScope } from "@/lib/compliance/compliance-derive";
...
export type ComplianceEnrichedItem = {
  id: string;
  domain: string;
  item_name: string;
  venue_scope: string;
  vendor_authority: string | null;
...
export async function fetchComplianceRegister(
  context: AuthContext,
  filter: ComplianceRegisterFilters = {},
): Promise<ComplianceEnrichedItem[]> {
  const { data, error } = await context.supabase
    .from("compliance_items_enriched")
    .select(
      "id, domain, item_name, venue_scope, vendor_authority, expiry_date, days_remaining, alert_tier, status, risk_level, renewal_cost, governing_date, item_type, license_number, notes",
    )
    .order("governing_date");
  if (error) throw error;
  let rows = (data ?? []) as ComplianceEnrichedItem[];
  if (filter.domain) rows = rows.filter((r) => r.domain === filter.domain);
  if (filter.vendor) {
    rows = rows.filter((r) =>
      String(r.vendor_authority ?? "").toLowerCase().includes(filter.vendor!.toLowerCase()),
    );
  }
...
export async function fetchComplianceRegisterKpis(
  context: AuthContext,
  filter: ComplianceRegisterFilters = {},
) {
  const items = await fetchComplianceRegister(context, filter);
  ...
  return { total, expired, due30, active, critical, healthPct };
}

export async function fetchComplianceCommandCenter(
  context: AuthContext,
  filter: ComplianceRegisterFilters = {},
) {
  const items = await fetchComplianceRegister(context, filter);
  ...
  const avgDays =
    withDays.length > 0
      ? Math.round(withDays.reduce((a, i) => a + Number(i.days_remaining), 0) / withDays.length)
      : 0;
  const healthPct = total > 0 ? Math.round(((total - expired - due30) / total) * 100) : 100;

  const byDomain = new Map<string, ComplianceEnrichedItem[]>();
  ...
  return {
    ...
    by_location: [
      ...
      {
        code: "All",
        name: "Shared (group-wide)",
        region: "—",
        total: shared.length,
        expired: shared.filter((i) => i.alert_tier === "Expired").length,
        due30: shared.filter((i) => i.alert_tier === "Due ≤30").length,
        ok: shared.filter((i) => i.alert_tier === "OK").length,
      },
    ],
...
export async function fetchComplianceAlerts(
  ...
  const missingOwner = items.filter((i) => !(i as { owner?: string }).owner).length;
  ...
  const flagged = items.map((i) => {
    ...
    else if (!(i as { owner?: string }).owner) flag = "✎ Missing owner";
...
export async function fetchComplianceCalendarMonth(
  context: AuthContext,
  params: { year: number; month: number; locationCode?: string | null },
) {
  const items = await fetchComplianceRegister(context, { locationCode: params.locationCode });
...
export async function fetchComplianceTrend(
  ...
  const { data: history } = await context.supabase
    .from("compliance_service_history")
    .select("service_date, cost");
  const months = Array.from({ length: 12 }, (_, i) => i + 1);
  let cumulative = 0;
  return {
    months: months.map((m) => {
      const prefix = `${params.year}-${String(m).padStart(2, "0")}`;
...
export async function fetchComplianceServiceHistory(
  context: AuthContext,
  filter: ServiceHistoryFilters = {},
) {
  ...
  if (filter.locationCode) q = q.eq("venue_scope", filter.locationCode);
  if (filter.result) q = q.eq("result", filter.result);
  if (filter.dateFrom) q = q.gte("service_date", filter.dateFrom);
  if (filter.dateTo) q = q.lte("service_date", filter.dateTo);
...
export async function fetchVendorScorecard(
  context: AuthContext,
  filter: { locationCode?: string | null } = {},
) {
  const items = await fetchComplianceRegister(context, { locationCode: filter.locationCode });
  const { data: history } = await context.supabase
    .from("compliance_service_history")
    .select("vendor, cost");
...
export async function fetchStaffReadiness(
  context: AuthContext,
  ...
  return {
    kpis: {
      ...
      staff_tracked: staff.length,
    },
    staff,
  };
}

export async function fetchComplianceCoverage(
  context: AuthContext,
  filter: ComplianceRegisterFilters = {},
) {
  const items = await fetchComplianceRegister(context, filter);
  ...
  const grid = domains.map((domain) => {
    ...
    const owners = [...new Set(domainItems.map((i) => (i as { owner?: string }).owner).filter(Boolean))];
    ...
    return {
      ...
      owner: owners[0] ?? "—",
      vendor: vendors[0] ?? "—",
      frequency: (domainItems[0] as { frequency?: string })?.frequency ?? "—",
      risk: domainItems.reduce(
        (max, i) => (i.risk_level === "Critical" ? "Critical" : max),
        "Low" as string,
      ),
    };
  });
...
export async function fetchSupervisorConsole(context: AuthContext, locationId: string) {
  ...
  const allItems = await fetchComplianceRegister(context, { locationCode: loc?.code ?? null });
...
export async function fetchExecutiveComplianceKpis(
  ...
  const items = await fetchComplianceRegister(context, filter);
  const staff = await fetchStaffReadiness(context, { locationId: null });
  const total = items.length;
  const expired = items.filter((i) => i.alert_tier === "Expired").length;
  const due30 = items.filter((i) => i.alert_tier === "Due ≤30").length;
  const healthPct = total > 0 ? Math.round(((total - expired - due30) / total) * 100) : 100;
  ...
  if (filter.locationCode) {
    const { data: loc } = await context.supabase
      .from("locations")
      .select("id")
      .eq("code", filter.locationCode)
      .maybeSingle();
    locationId = loc?.id ?? null;
  }
  const sup = locationId ? await fetchSupervisorConsole(context, locationId) : null;
  const openingPct = sup?.kpis.opening_readiness_pct ?? 100;
  const issueClosure =
    sup && sup.kpis.logged_today > 0
