...
export default function EmployeeMePage() {
  ...
  const linked = Boolean(ctx.data?.staff?.id);
  ...
  const ctxReady = !ctx.isLoading;
  const unlinked = ctxReady && !linked;
            ...
            <section className="hr-panel-shell hr-enter">
              <div className="hr-panel p-4">
                <h1 className="text-lg font-semibold tracking-tight">
                  {ctx.data?.staff?.fullName ?? t("common.loading")}
                </h1>
                {ctx.data?.staff?.employeeCode ? (
            ...
            <section className="hr-panel-shell">
              <div className="hr-panel p-4">
                ...
                <div className="mt-2 space-y-2">
                  {!attendance.isLoading && (attendance.data?.rows ?? []).length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("hr.me.noStatus")}</p>
                  ) : null}
                  {(attendance.data?.rows ?? []).slice(0, 7).map((row) => (
              ...
              <div className="hr-panel space-y-3 p-4">
                ...
                {(leave.data ?? []).slice(0, 12).map((row) => (
                  <div key={row.id} className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                    <span>
                      {row.dateFrom} → {row.dateTo} · {t(`hr.leave.types.${row.leaveType}`)} ·{" "}
                      {t(`hr.leave.status.${row.status}`)} · {row.days}d
                    </span>
                    {row.status === "pending" ? (
                      <Button
                        size="sm"
