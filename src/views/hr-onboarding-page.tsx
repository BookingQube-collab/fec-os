      ...
      <HrShell>
        ...
        >
          ...
          <div className="space-y-3">
            {(lists.data ?? []).length === 0 ? (
              <HrPanel delay={1}>
                <HrEmptyState message={t("hr.onboarding.empty")} icon={ClipboardList} />
              </HrPanel>
            ) : (
              (lists.data ?? []).map((row, i) => {
                const progress = checklistProgress(row.items);
                return (
                  <HrPanel key={row.id} delay={i + 1}>
                    <div className="space-y-3 p-4 sm:p-5">
                      ...
                      <div className="h-1.5 overflow-hidden rounded-full bg-[var(--hr-cream-deep)]">
                        <div
                          className="h-full rounded-full bg-[var(--hr-mustard)] transition-[width] duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]"
                          style={{ width: `${progress.percent}%` }}
                        />
                      </div>
                      <ul className="space-y-2">
                        {row.items.map((item) => (
                          <li key={item.id} className="hr-list-row text-sm">
                            <span>{item.title}</span>
                            <div className="flex gap-2">
                              <Badge
                                variant={
                                  item.status === "done" ? "success" : item.status === "skipped" ? "outline" : "muted"
                                }
                              >
                              ...
                              {row.status === "open" && item.status === "pending" ? (
                                <>
                                  <Button
                                    size="sm"
                                    variant="secondary"
                                    onClick={() => toggleItem.mutate({ itemId: item.id, status: "done" })}
                                  >
                                  ...
                                  <Button
                                    size="sm"
                                    variant="ghost"
                                    onClick={() => toggleItem.mutate({ itemId: item.id, status: "skipped" })}
                                  >
