...
import { fetchComplianceTrend } from "@/lib/queries/compliance-register.core";
...
export async function GET(request: Request) {
  const response = await withAuthRouteRequest(
    async (context, req) => {
      ...
      const cacheKey = routeCacheKey(["compliance-trend", context.userId, JSON.stringify(filters)]);
      const cached = getRouteCache<Awaited<ReturnType<typeof fetchComplianceTrend>>>(cacheKey);
      if (cached) return cached;
      const payload = await fetchComplianceTrend(context, filters as never);
