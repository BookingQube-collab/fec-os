...
import { fetchComplianceAlerts } from "@/lib/queries/compliance-register.core";
...
export async function GET(request: Request) {
  const response = await withAuthRouteRequest(
    async (context, req) => {
      ...
      const cacheKey = routeCacheKey(["compliance-alerts", context.userId, JSON.stringify(filters)]);
      const cached = getRouteCache<Awaited<ReturnType<typeof fetchComplianceAlerts>>>(cacheKey);
      if (cached) return cached;
      const payload = await fetchComplianceAlerts(context, filters as never);
