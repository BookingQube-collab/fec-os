...
import { fetchVendorScorecard } from "@/lib/queries/compliance-register.core";
...
export async function GET(request: Request) {
  const response = await withAuthRouteRequest(
    async (context, req) => {
      ...
      const cacheKey = routeCacheKey(["compliance-vendor-scorecard", context.userId, JSON.stringify(filters)]);
      const cached = getRouteCache<Awaited<ReturnType<typeof fetchVendorScorecard>>>(cacheKey);
      if (cached) return cached;
      const payload = await fetchVendorScorecard(context, filters as never);
