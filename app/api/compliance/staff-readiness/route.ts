...
import { fetchStaffReadiness } from "@/lib/queries/compliance-register.core";
...
export async function GET(request: Request) {
  const response = await withAuthRouteRequest(
    async (context, req) => {
      ...
      const cacheKey = routeCacheKey(["compliance-staff-readiness", context.userId, JSON.stringify(filters)]);
      const cached = getRouteCache<Awaited<ReturnType<typeof fetchStaffReadiness>>>(cacheKey);
      if (cached) return cached;
      const payload = await fetchStaffReadiness(context, filters as never);
