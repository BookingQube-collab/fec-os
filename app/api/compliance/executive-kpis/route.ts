...
import { fetchExecutiveComplianceKpis } from "@/lib/queries/compliance-register.core";
...
export async function GET(request: Request) {
  const response = await withAuthRouteRequest(
    async (context, req) => {
      ...
      const cacheKey = routeCacheKey(["compliance-executive-kpis", context.userId, JSON.stringify(filters)]);
      const cached = getRouteCache<Awaited<ReturnType<typeof fetchExecutiveComplianceKpis>>>(cacheKey);
      if (cached) return cached;
      const payload = await fetchExecutiveComplianceKpis(context, filters as never);
