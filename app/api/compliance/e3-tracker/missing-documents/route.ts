...
import {
  fetchE3TrackerMissingDocuments,
  logE3TrackerPerf,
...
export async function GET(request: Request) {
  return withAuthRouteRequest(
    async (context, req) => {
      ...
      const payload = await fetchE3TrackerMissingDocuments(context, filters);
      const dbMs = performance.now() - dbStart;
      const handlerMs = performance.now() - handlerStart;
      logE3TrackerPerf("missing-documents", payload.items.length, handlerMs, dbMs);
      return payload;
    },
    request,
