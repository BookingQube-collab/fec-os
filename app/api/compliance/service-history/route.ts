...
import { fetchComplianceServiceHistory } from "@/lib/queries/compliance-register.core";
...
export async function GET(request: Request) {
  const response = await withAuthRouteRequest(
    async (context, req) => {
      ...
      const cacheKey = routeCacheKey(["compliance-service-history", context.userId, JSON.stringify(filters)]);
      const cached = getRouteCache<Awaited<ReturnType<typeof fetchComplianceServiceHistory>>>(cacheKey);
      if (cached) return cached;
      const payload = await fetchComplianceServiceHistory(context, filters as never);
